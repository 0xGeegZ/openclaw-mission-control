...
These artifacts are **generated locally** and **not tracked in git**.
...
`npm run dev`, `npm run seed`, and `npm run deploy` only run a fail-fast check to ensure the generated artifacts are present and up to date.
Use `npm run seed:refresh` (or `npm run seed-skills:sync`) when you need to refresh skill sources or regenerate artifacts.

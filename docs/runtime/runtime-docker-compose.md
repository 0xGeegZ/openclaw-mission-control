...
**Where to get SERVICE_TOKEN and ACCOUNT_ID:** Sign in to the web app → your account → **OpenClaw** (admin) → generate a service token. For ACCOUNT_ID: [Convex Dashboard](https://dashboard.convex.dev) → your deployment → Data → `accounts` table → copy the `_id`. See the root [README](../../README.md) [Run the runtime locally (optional)](../../README.md#run-the-runtime-locally-optional) section.
...
| `OPENCLAW_REQUEST_TIMEOUT_MS`      | No           | Timeout for `/v1/responses` requests in ms (default `300000`). Clamped to 5000–600000 if out of range.                                                                                                                                                           |
| `OPENCLAW_WORKSPACE_ROOT`          | No           | Root directory for per-agent OpenClaw workspaces (default `/root/clawd/agents`). Runtime writes USER.md, IDENTITY.md, SOUL.md, TOOLS.md, AGENTS.md, HEARTBEAT.md when profile sync is enabled. Other profile-sync paths are derived internally from this folder. |
...
| `OPENCLAW_BROWSER_EXECUTABLE_PATH` | No           | Optional managed-browser executable override for `openclaw-gateway` startup. If unset, startup auto-detects in this order: `brave-browser`, `chromium`, `chromium-browser`.                                                                                      |
...
**Volume permissions:** When you run `npm run dev:openclaw`, the script `scripts/runtime-docker-up.sh` runs **every time** (no one-time steps): it creates the workspace dirs, writes `DOCKER_UID`/`DOCKER_GID` to `apps/runtime/.env.docker-uid` so Compose runs the runtime as your host user, runs `chown -R $(id -u):$(id -g) .runtime/openclaw-workspace`, then `chmod -R a+rwX` so the gateway (root) can also write (e.g. `MEMORY.md`, `memory/WORKING.md`). After a Docker restart, run `npm run dev:openclaw` again; the script will re-apply chown and chmod. If you see `EACCES: permission denied, mkdir '/root/clawd/agents'` in the runtime logs, run from the repo root:

```bash
sudo chown -R $(id -u):$(id -g) .runtime/openclaw-workspace
sudo chmod -R a+rwX .runtime/openclaw-workspace
```

If the gateway logs `touch: cannot touch '/root/clawd/MEMORY.md': Permission denied`, run the same `chmod` (and `chown` if needed), then restart the stack.
...
- **`EACCES: permission denied, mkdir '/root/clawd/agents'` (AgentSync)**  
  This usually indicates a legacy runtime mount/path (runtime writing under `/root/*` as non-root). Ensure you are using the current Compose config where runtime uses `/clawd` (`OPENCLAW_WORKSPACE_ROOT=/clawd/agents`). Then re-run `npm run dev:openclaw` so the script re-applies chown and chmod. If needed, from repo root run: `sudo chown -R $(id -u):$(id -g) .runtime/openclaw-workspace` and `sudo chmod -R a+rwX .runtime/openclaw-workspace`, then restart. See **Volume permissions** under Volumes (OpenClaw).

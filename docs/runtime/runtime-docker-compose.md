...
| `OPENCLAW_WORKSPACE_ROOT`          | No           | Root directory for per-agent OpenClaw workspaces (default `/root/clawd/agents`). Runtime writes SOUL.md, TOOLS.md, AGENTS.md here when profile sync is enabled. Other profile-sync paths (generated config and manual fallbacks) are derived internally from this folder. |

...
| `OPENCLAW_WORKSPACE_ROOT`         | No           | Root directory for per-agent OpenClaw workspaces (default `/root/clawd/agents`). Runtime writes SOUL.md, TOOLS.md, AGENTS.md here when profile sync is enabled. Set `OPENCLAW_PROFILE_SYNC=true` to populate these files. Must be writable and shared with gateway when using profile sync. |

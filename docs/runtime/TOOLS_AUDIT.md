...
| **document_list**    | When canCreateDocuments or hasTaskContext | List documents for account or task (optional taskId, type, limit). Discover existing docs before creating or referencing.                  |
...
| **P1**   | **document_list**                                                                                                        | Prevents duplicate docs and enables “reference existing doc” behavior. | Done: Convex service action listDocumentsForAgent + internal query listForAgentTool; runtime tool in agentTools when canCreateDocuments or hasTaskContext; documented in AGENTS.md and this table. |

...
if (taskId && !responseText?.trim()) {
  log.debug(
    "No response text to write back for notification",
    notification._id,
  );
}
```
...
- `OPENCLAW_REQUEST_TIMEOUT_MS` (default 180s) is applied to the fetch via `AbortController`; no minimum clamp. If someone sets a very low or negative value, `parseIntOrDefault` can return it; consider clamping to e.g. 5_000–300_000 ms in config if you want to harden misconfiguration.

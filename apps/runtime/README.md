...
| `DELIVERY_MAX_CONCURRENT_SESSIONS`                    | No       | Max concurrent session streams per delivery cycle (default `10`). Clamped to 1–100. Should be ≤ OpenClaw gateway capacity.                                                                                                                                                                                                                                                                                                                   |
| `DELIVERY_STREAM_TIMEOUT_MS`                          | No       | Per-session stream timeout in ms (default min(2×OPENCLAW_REQUEST_TIMEOUT_MS, 300000)). Clamped to 5000–300000. Use ≥ 300000 (5 min) when subagents are used so aggregate-before-reply does not time out.                                                                                                                                                                                                                                     |
| `DELIVERY_CONTEXT_FETCH_BATCH_SIZE`                   | No       | Batch size for parallel getNotificationForDelivery fetches (default `15`). Clamped to 1–50.                                                                                                                                                                                                                                                                                                                                                  |
| `DELIVERY_LIST_LIMIT`                                 | No       | Max notifications to list per poll cycle (default `50`). Clamped to 1–500. Tune with DELIVERY_MAX_CONCURRENT_SESSIONS for throughput.                                                                                                                                                                                                                                                                                                        |

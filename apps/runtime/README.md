...
2. Starts the **OpenClaw gateway** and registers agent sessions. Session keys are resolved by the backend: **task-scoped** (`task:{taskId}:agent:{slug}:{accountId}:vN`) for notifications with a task, and **system** (`system:agent:{slug}:{accountId}:vN`) for non-task (e.g. heartbeat). One persistent session per (task, agent) while the task is active; a new generation is created when a task is reopened after being done.
3. **Delivery loop**: polls Convex for undelivered agent notifications (mentions, subscriptions, assignments), then sends them to the correct OpenClaw session via the OpenResponses HTTP API (`POST {OPENCLAW_GATEWAY_URL}/v1/responses` with `x-openclaw-session-key`). Each request sends **instructions** (system/policy) and **input** (notification payload + compact context) separately. Agent replies are **written back** to the task thread in Convex so they appear in the OpenClaw Mission Control web app. The gateway HTTP endpoint must be enabled in OpenClaw config. All Convex service actions invoked by the delivery loop use `config.serviceToken` and `config.accountId`; account scope is enforced by `getNotificationForDelivery`.
...
| `DELIVERY_MAX_CONCURRENT_SESSIONS`                    | No       | Max concurrent session streams per poll cycle (default `10`). Clamped to 1–100. Start at 10; increase only if gateway and Convex are not saturated. High values (e.g. 50–100) can stress both. Delivery is parallelized by `deliverySessionKey` so one agent can work on multiple tasks at once. **Keep `OPENCLAW_MAX_CONCURRENT` ≥ this value** so the gateway can process all streams; otherwise excess streams queue at the gateway.      |
| `DELIVERY_STREAM_TIMEOUT_MS`                          | No       | Per-session-stream timeout in ms (default: min(2× `OPENCLAW_REQUEST_TIMEOUT_MS`, 5 min)). One stuck stream does not block the cycle. Clamped to 5000–(2× openclaw timeout). Should be at least as large as a single long-running agent response.                                                                                                                                                                                             |
| `DELIVERY_CONTEXT_FETCH_BATCH_SIZE`                   | No       | Batch size for parallel `getNotificationForDelivery` fetches (default `15`). Clamped to 1–50. Higher values shorten time-to-first-delivery when there are many notifications; ensure Convex can handle the load.                                                                                                                                                                                                                             |
| `DELIVERY_LIST_LIMIT`                                 | No       | Max notifications to list per poll cycle (default `50`). Clamped to 1–500. Backend allows up to 500; increase when using high concurrency to drain backlog faster.                                                                                                                                                                                                                                                                           |
...
| `OPENCLAW_MAX_CONCURRENT`                             | No       | OpenClaw gateway `agents.defaults.maxConcurrent`: max parallel agent runs across sessions (default `10`). Clamped to 1–16. Enables Lead to respond in orchestrator chat and task threads at once; OpenClaw default is 1. Applied by the gateway startup script; env wins over runtime-generated config. **Should be ≥ `DELIVERY_MAX_CONCURRENT_SESSIONS`** so the gateway can handle all concurrent delivery streams (defaults align at 10). |
| `AI_GATEWAY_API_KEY`                                  | No       | Vercel AI Gateway key used by OpenClaw (optional). If unset, `VERCEL_AI_GATEWAY_API_KEY` is used.                                                                                                                                                                                                                                                                                                                                            |
| `BRAVE_API_KEY`                                       | No       | Brave Search API key for OpenClaw `web_search` (optional). When set in the runtime/gateway environment (e.g. Docker `env` or `.env`), the startup script enables Brave as the web search provider. Get a key at [brave.com/search/api](https://brave.com/search/api/) (use the **Data for Search** plan; Data for AI is not compatible with web_search).                                                                                     |

...
| `DELIVERY_MAX_CONCURRENT_SESSIONS`                    | No       | Max concurrent session streams per poll cycle (default `10`). Clamped to 1–100. Delivery is parallelized by `deliverySessionKey` so one agent can work on multiple tasks at once.                                                                                                                                                                        |
| `DELIVERY_STREAM_TIMEOUT_MS`                          | No       | Per-session-stream timeout in ms (default: 2× `OPENCLAW_REQUEST_TIMEOUT_MS`). One stuck stream does not block the cycle. Clamped to 5000–(2× openclaw timeout).                                                                                                                                                                                          |

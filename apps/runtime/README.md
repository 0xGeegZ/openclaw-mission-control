...
| `OPENCLAW_MAX_CONCURRENT`                             | No       | OpenClaw gateway `agents.defaults.maxConcurrent`: max parallel agent runs across sessions (default `5`). Clamped to 1–16. Enables Lead to respond in orchestrator chat and task threads at once; OpenClaw default is 1. Applied by the gateway startup script; env wins over runtime-generated config.                                                   |

...
function formatNotificationMessage(
  ...
  const statusInstructions = task
    ? canModifyTaskStatus
      ? `If you need to change task status, do it BEFORE posting a thread update. Only move to a valid next status from the current one (assigned -> in_progress, in_progress -> review, review -> done or back to in_progress; use blocked only when blocked). Do not move directly to done unless the current status is review. You have the **task_status** tool (in your Capabilities) — call it with taskId, status (in_progress|review|done|blocked), and blockedReason when status is blocked. If your interface does not show that tool, use the HTTP fallback: POST ${taskStatusBaseUrl.replace(/\/$/, "")}/agent/task-status with header \`x-openclaw-session-key: ${context.agent?.sessionKey ?? ""}\` and JSON body \`{ "taskId": "<Task ID above>", "status": "<next valid status>", "blockedReason": "..." }\`. Note: inbox/assigned are handled by assignment changes, not this tool. If a tool returns an error, do not claim you changed status; report BLOCKED. If you have no way to update status (both tool and HTTP fail), do not post a completion summary; report BLOCKED and state that you could not update task status.`
